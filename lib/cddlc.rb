                             ...
                             if preferred_tag
                               from.map{ |k|
                                 if k =~ /\A#{preferred_tag}[.]/
                                   ...
                                   if old = ret.store_rule(k, v = ["name", nn])
                                     warn "*** autogenerated rule #{k} = #{v} would overwrite #{old}"
                                   end
                                   nn
                                 else
                                   ret.warn_error "*** imported rule #{k.inspect} not in #{fn}"
                                 end
                               }.compact
                             else
                               must_be_found = true
          ...
          loop do
            ...
            to_be_imported.each do |name|
              if rule = included_cddl.rules[name]
                ...
                ret.warn_error "*** imported rule #{name.inspect} not in #{fn}"
              end
            end
            break unless got_more
            to_be_imported = ret.cddl_undefined # XXX square...
            must_be_found = false
          end
          if preferred_tag
            to_be_imported.each do |name|
